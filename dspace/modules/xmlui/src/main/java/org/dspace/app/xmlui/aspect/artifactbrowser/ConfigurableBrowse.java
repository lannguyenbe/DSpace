        ...
        CacheableProcessingComponent
{
        ...
        if (itemsTotal > 0) 
        {
            ...
            else    // browsing a list of unique metadata entries
            {
                ...
                for (String[] singleEntry : browseInfo.getStringResults())
                {
                    ...
                    if (isFacetBrowse(info)) { // Lan 17.02.2015 - Facet browse
                    	// in facet browse, links point to Advanced Search as a sidebar facet 
	                    cell.addXref(
                                contextPath +
	                                "/discover?" +
	                                "filtertype=" + encodeForURL(type) +
	                                "&filter_relational_operator=equals" +
	                                "&filter=" + encodeForURL(singleEntry[0]),
		                        singleEntry[0]);                    	
                    } else { 
                    	// classical browse link
    ...
    private boolean isFacetBrowse(BrowseInfo info)
    {
    	return "facet".equals(info.getBrowseIndex().getDataType());
    }
